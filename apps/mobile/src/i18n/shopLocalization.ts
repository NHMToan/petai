...
type LocalizedProductText = {
  ...
  badge: string;
  category?: string;
  specs?: NonNullable<ShopItem["specs"]>;
};
...
export function localizeShopItem(item: ShopItem, locale: AppLocale): ShopItem {
  const localized = productTranslations[item.slug]?.[locale];
  ...
  return {
    ...
    specs: localized.specs ?? item.specs,
  };
}
